...
public class SwiftMobileSmartWatchPlugin: NSObject, FlutterPlugin,  FlutterStreamHandler{
    ...
    override init() {
        ...
        print(Constants.GET_LAST_DEVICE_ADDRESS)
    }
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let methodChannel = FlutterMethodChannel(name: Constants.SMART_METHOD_CHANNEL, binaryMessenger: binaryMessenger)
        
        let eventChannel = FlutterEventChannel(name: Constants.SMART_EVENT_CHANNEL, binaryMessenger: binaryMessenger)
        let bpEventChannel = FlutterEventChannel(name: Constants.SMART_BP_TEST_CHANNEL, binaryMessenger: binaryMessenger)
        let oxygenEventChannel = FlutterEventChannel(name: Constants.SMART_OXYGEN_TEST_CHANNEL, binaryMessenger: binaryMessenger)
        let tempEventChannel = FlutterEventChannel(name: Constants.SMART_TEMP_TEST_CHANNEL, binaryMessenger: binaryMessenger)
        
        let callBackChannel = FlutterMethodChannel(name: Constants.SMART_CALLBACK, binaryMessenger: binaryMessenger)
